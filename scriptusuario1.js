...
// Função para preencher as opções de unidade nos filtros
function fetchUnitsAndSectors() {
    ...
    unitSelect.innerHTML = '<option value="">Selecione Unidade</option>';
        ...
        sectors[sector].sort((a, b) => a.name.localeCompare(b.name)).forEach(person => {
            ...
            extensionElement.addEventListener('click', (e) => {
                ...
                const drag = (e) => {
                    ...
                    currentX = Math.min(Math.max(0, diff), maxX);
                    ...
                    const hue = 120 * (1 - progress); // Green to red
                    slideContainer.style.backgroundColor = `hsl(${hue}, 70%, 45%)`;
                    
                    // Update call text
                    if (progress > 0.7) {
                        callText.textContent = 'Solte para ligar';
                    } else {
                        callText.textContent = 'Arraste para ligar';
                    }
                };
                ...
                const endDrag = () => {
                    ...
                    // Check if dragged enough to trigger call
                    const progress = currentX / maxX;
                ...
                // Add the popup to the extension element
                extensionElement.innerHTML = '';
                extensionElement.appendChild(slideContainer);
                
                // Add click outside to close
                const handleClickOutside = (e) => {
                    if (!extensionElement.contains(e.target)) {
                        extensionElement.innerHTML = originalExtensionContent;
